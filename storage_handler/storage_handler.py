...
class StorageHandler(ABC):
    ...
    @abstractmethod
    def list_files(self, prefix: PosixPath = "", relative: bool = True) -> List[str]:
        """
        List files under a given prefix.

        Args:
            prefix (PosixPath): The prefix to filter files.
    ...
    @abstractmethod
    def list_files_recursive(self, prefix: PosixPath = "", relative: bool = True) -> List[str]:
        """
    ...
    @abstractmethod
    def glob_files(self, pattern: PosixPath, relative: bool = True) -> List[str]:
        """
        Glob files matching a pattern.

        Args:
            pattern (PosixPath): The glob pattern to match files.
    ...
    @abstractmethod
    def rename(self, old_remote_path: PosixPath, new_remote_path: PosixPath, relative: bool = True) -> None:
        """
        Rename a remote file.

        Args:
            old_remote_path (PosixPath): Current path to the remote file.
            new_remote_path (PosixPath): New path for the remote file.
            relative (bool): Whether the paths are relative to the base path.
        """
        pass

    @abstractmethod
    def copy(self, source_remote_path: PosixPath, destination_remote_path: PosixPath, relative: bool = True) -> None:
        """
        Copy a remote file to another location within the same storage.

        Args:
            source_remote_path (PosixPath): Path to the source remote file.
            destination_remote_path (PosixPath): Path to the destination remote file.
            relative (bool): Whether the paths are relative to the base path.
        """
        pass
    ...
    @abstractmethod
    def get_file_metadata(self, remote_path: PosixPath, relative: bool = True) -> Optional[dict]:
        """
